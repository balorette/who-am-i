...
Next.js 14+ App Router supports static export with `output: 'export'` in next.config.js. This generates a fully static site that can be hosted on any static file server (S3, Netlify, Vercel, GitHub Pages, etc.).
...
- Serverless functions if needed (Vercel Functions, AWS Lambda)
...
Using this for my personal site. Content lives in markdown files, builds to static HTML, deployed to AWS S3 with CloudFront. Total hosting cost: ~$1/month for millions of page views.

The best part? Still get all the DX benefits of Next.js (TypeScript, hot reload, React) without the complexity of a server.
